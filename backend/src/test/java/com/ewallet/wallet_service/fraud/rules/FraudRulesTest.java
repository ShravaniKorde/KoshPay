...
class FraudRulesTest {
    ...
    void testWalletDrainPercentageRule() {
        ...
        FraudContext zeroBalance = new FraudContext(1L, 2L, new BigDecimal("10"));
        zeroBalance.setCurrentBalance(BigDecimal.ZERO);
        assertFalse(rule.isTriggered(zeroBalance));
    ...
    void testNewPayeeRule() {
        ...
        when(transactionRepository.existsByFromWalletIdAndToWalletId(1L, 2L)).thenReturn(false);
        assertTrue(rule.isTriggered(new FraudContext(1L, 2L, BigDecimal.TEN)));

        when(transactionRepository.existsByFromWalletIdAndToWalletId(1L, 3L)).thenReturn(true);
